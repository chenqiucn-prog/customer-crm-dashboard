...
async function saveProject(){try{const p=collectProject(); if(p.id){await api("/api/projects",{method:"PUT",body:JSON.stringify(p)});}else{await api("/api/projects",{method:"POST",body:JSON.stringify(p)});} closeDrawer(); showToast("项目已保存。"); await loadAll();}catch(e){showToast(e.message)}}
...
function bind(){
  ...
  $("refreshBtn").addEventListener("click",loadAll); $("exportBtn").addEventListener("click",exportJSON); $("newBtn").addEventListener("click",()=>openDrawer()); $("closeDrawerBtn").addEventListener("click",closeDrawer); $("saveProjectBtn").addEventListener("click",saveProject); $("f_productLine").addEventListener("change",syncProductOptions); $("resetFilterBtn").addEventListener("click",clearFilters);
